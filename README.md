...
**Supported file types:** `.docx`, `.pptx`, `.xlsx`, `.xls`, `.pdf`, `.html`, `.htm`, `.csv`, `.json`, `.xml`, `.eml`, `.txt`, `.md` — markitdown extras are installed dynamically based on which file types are present in `source\`; `.eml` files are converted via Python stdlib (no pip install needed)
