...
- **`.eml` file support** — email files (`.eml`) converted via Python stdlib `email` module (no pip install needed). Extracts subject, from, to, cc, date, and body text into markdown. Requires venv (for Python) but no markitdown. New "Stdlib conversion" group in the dependency classification table
